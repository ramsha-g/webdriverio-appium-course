...
async function createSim(caps) {
    ...
    const platform = (0, utils_1.normalizePlatformName)(caps.platformName);
    ...
    logger_1.default.debug(`Creating a temporary Simulator device '${simName}'`);
...
async function getExistingSim(opts = /** @type {SimulatorLookupOptions} */ ({})) {
    ...
    const platform = (0, utils_1.normalizePlatformName)(opts.platformName);
    ...
    if (udid && lodash_1.default.toLower(udid) !== utils_1.UDID_AUTO) {
        logger_1.default.debug(`Looking for an existing Simulator with UDID '${udid}'`);
    ...
    if (!platformVersion) {
        logger_1.default.debug(`Provide 'platformVersion' capability if you prefer an existing Simulator to be selected`);
        return null;
    }
    const devices = devicesMap?.[platformVersion] ?? await simctl.getDevices(platformVersion, platform);
    logger_1.default.debug(`Looking for an existing Simulator with platformName: ${platform}, ` +
        `platformVersion: ${platformVersion}, deviceName: ${deviceName}`);
    for (const device of devices) {
        if ((deviceName && device.name === deviceName) || !deviceName) {
            if (!deviceName) {
                logger_1.default.debug(`The 'deviceName' capability value is empty. ` +
