...
exports.requireArgs = exports.clearLogs = exports.normalizePlatformVersion = exports.isLocalHost = exports.removeAllSessionWebSocketHandlers = exports.encodeBase64OrUpload = exports.getPIDsListeningOnPort = exports.printUser = exports.markSystemFilesForCleanup = exports.DEFAULT_TIMEOUT_KEY = exports.normalizeCommandTimeouts = exports.translateDeviceName = exports.clearSystemFiles = exports.getDriverInfo = exports.checkAppPresent = exports.getAndCheckIosSdkVersion = exports.getAndCheckXcodeVersion = exports.detectUdid = exports.normalizePlatformName = exports.isTvOs = exports.assertSimulator = exports.UDID_AUTO = void 0;
...
async function detectUdid() {
    logger_1.default.debug('Auto-detecting real device udid...');
    ...
    if (udids.length > 1) {
        logger_1.default.warn(`Multiple devices found: ${udids.join(', ')}`);
        logger_1.default.warn(`Choosing '${udid}'. If this is wrong, manually set with 'udid' desired capability`);
    }
    logger_1.default.debug(`Detected real device udid: '${udid}'`);
    return udid;
}
...
async function getAndCheckIosSdkVersion() {
    try {
        ...
        logger_1.default.errorAndThrow(`Could not determine iOS SDK version: ${err.message}`);
    }
}
...
async function clearSystemFiles(wda) {
    ...
    if (lodash_1.default.isEmpty(dstFolders)) {
        ...
        for (const dstFolder of dstFolders) {
            ...
            bluebird_1.default.resolve(support_1.fs.walkDir(dstFolder, true, (itemPath, isDir) => {
                if (isDir) {
                    return;
                }
                ...
                support_1.fs.unlink(itemPath).catch((e) => {
                ...
                if (scheduledFilesCount > 0) {
                    logger_1.default.info(`Scheduled ${scheduledFilesCount} temporary XCTest log ` +
                        `${support_1.util.pluralize('file', scheduledFilesCount)} for cleanup in '${dstFolder}'`);
                }
                // eslint-disable-next-line promise/prefer-await-to-then
            })
                .catch((e) => {
        ...
        logger_1.default.debug(`Started background XCTest logs cleanup in '${dstFolders}'`);
    }
...
async function checkAppPresent(app) {
    ...
    if (!(await support_1.fs.exists(app))) {
        logger_1.default.errorAndThrow(`Could not find app at '${app}'`);
    }
    logger_1.default.debug('App is present');
}
exports.checkAppPresent = checkAppPresent;
async function getDriverInfo() {
    const stat = await support_1.fs.stat(node_path_1.default.resolve(__dirname, '..'));
    const built = stat.mtime.getTime();
    // get the package.json and the version from it
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const pkg = support_1.fs.readPackageJsonFrom(support_1.fs.findRoot(__filename));
...
function normalizeCommandTimeouts(value) {
    ...
    try {
        ...
        logger_1.default.errorAndThrow(`"commandTimeouts" capability should be a valid JSON object. "${value}" was given instead`);
    }
    for (let [cmd, timeout] of lodash_1.default.toPairs(result)) {
        if (!lodash_1.default.isInteger(timeout) || timeout <= 0) {
            logger_1.default.errorAndThrow(`The timeout for "${cmd}" should be a valid natural number of milliseconds. "${timeout}" was given instead`);
        }
    }
    return result;
}
...
async function encodeBase64OrUpload(localPath, remotePath = null, uploadOptions = {}) {
    if (!(await support_1.fs.exists(localPath))) {
        logger_1.default.errorAndThrow(`The file at '${localPath}' does not exist or is not accessible`);
    }
