...
exports.gesturesChainToString = void 0;
...
const commands = {
    ...
    async moveTo(el, xoffset = 0, yoffset = 0) {
        ...
        if (this.isWebContext()) {
            ...
            if (lodash_1.default.isNil(el)) {
                if (!this.curCoords) {
                    throw new driver_1.errors.UnknownError('Current cursor position unknown, please use moveTo with an element the first time.');
                }
                this.curCoords = {
    ...
    async performMultiAction(actions) {
        ...
        for (let i in actions) {
            this.log.debug(`    ${parseInt(i, 10) + 1}: ${lodash_1.default.map(actions[i], 'action').join('-')}`);
        }
        try {
