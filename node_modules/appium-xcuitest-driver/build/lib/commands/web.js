    ...
    try {
        ...
        if (text) {
            ...
            if (els.length === 1 || els.length === 2) {
                ...
                const rect = await driver.proxyCommand(`/element/${support_1.util.unwrapElement(el)}/rect`, 'GET');
                if (els.length === 2) {
                    const el2 = els[1];
                    const rect2 = await driver.proxyCommand(`/element/${support_1.util.unwrapElement(el2)}/rect`, 'GET');
                    if (rect.x !== rect2.x ||
                        rect.y !== rect2.y ||
...
const commands = {
    ...
    async setCookie(cookie) {
        ...
        const jsCookie = cookies_1.default.createJSCookie(cookie.name, cookie.value, {
...
const extensions = {
    ...
    async getExtraTranslateWebCoordsOffset(wvPos, realDims) {
        ...
        const { nativeWebTapTabBarVisibility, nativeWebTapSmartAppBannerVisibility, 
        // @ts-expect-error - do not assign arbitrary properties to `this.opts`
        safariTabBarPosition = support_1.util.compareVersions(this.opts.platformVersion, '>=', '15.0') &&
            isIphone
    ...
    async nativeWebTap(el) {
        ...
        if (!(await this.settings.getSettings()).nativeWebTapStrict &&
        ...
        await bluebird_1.default.Promise.all([
    ...
    async waitForAtom(promise) {
        ...
        const timedAtomPromise = bluebird_1.default.resolve(promise).timeout(ATOM_WAIT_TIMEOUT_MS);
        const handlePromiseError = async (p) => {
            try {
                return await p;
            }
            catch (err) {
                const originalError = err instanceof bluebird_1.default.AggregateError ? err[0] : err;
                this.log.debug(`Error received while executing atom: ${originalError.message}`);
                if (originalError instanceof bluebird_1.default.TimeoutError) {
                    throw new driver_1.errors.TimeoutError(`Did not get any response for atom execution after ` +
